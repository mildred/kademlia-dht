...
Lookup.prototype._forContact = function (contact, cb) {
    var self = this;
    this._opts.findNodeOrValue(contact, this._targetId, this._idkey, this._subkey, function (err, contacts, value, valueTimeout) {
        if(self._abort) {
            --self._concurrents;
            return;
        }
        ...
        if (err) {
            ...
            if (value) {
                if (self._subkey !== null) {
                    ...
                    cb(null, self._list.getContacts(), value, valueTimeout, contact);
                    ...
                    for(var k in value) {
                        ...
                        self._valueTimeout[k] = valueTimeout[k];
                    }
                }
            }
        }
