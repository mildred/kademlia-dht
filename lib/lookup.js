...
//
function Lookup(id, key, subkey, seeds, opts) {
...
// requests.
//
Lookup.proceed = function (targetId, key, subkey, seeds, opts, cb) {
    ...
    var lookup = new Lookup(targetId, key, subkey, seeds, opts);
    lookup.proceed(cb);
    return lookup;
};
...
Lookup.prototype._forContact = function (contact, cb) {
    var self = this;
    this._opts.findNodeOrValue(contact, this._targetId, this._key, this._subkey, function (err, contacts, value, timeout) {
        if(self._abort) {
            --self._concurrents;
            return;
        }
