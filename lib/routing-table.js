...
RoutingTable.prototype = new events.EventEmitter;
...
// Get the contact `id` or return undefined
RoutingTable.prototype.getContact = function (id) {
    if (!(id instanceof Id))
        throw new Error('invalid id');
    var res = this._findBucket(id);
    return res.bucket.getContact(id);
};
...
//
RoutingTable.prototype._find = function (id, rank, node, count, list, filter) {
    if (node instanceof Bucket) {
        var nodelist = node.obtain();
        if(filter) nodelist = nodelist.filter(filter);
        list.insertMany(nodelist);
        return;
    }
    var self = this;
    function findIn(main, other) {
        self._find(id, rank + 1, main, count, list, filter);
        if (list.length < count)
            self._find(id, rank + 1, other, count, list, filter);
    }
    if (id.at(rank)) {
...
// Get the `count` known contacts closest from `id`. If `filter` is defined,
// then the contacts must return true to `filter(contact)`
...
//
RoutingTable.prototype.find = function (id, count, filter) {
    if (!(id instanceof Id))
        throw new Error('invalid id');
    if (typeof count === 'function') {
        filter = count;
        count = this._bucketSize;
    }
    if (typeof count === 'undefined')
        count = this._bucketSize;
    var list = new LookupList(id, count);
    this._find(id, 0, this._root, count, list, filter);
