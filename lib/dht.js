...
Dht.prototype._storeToCache = function (key, subkey, value, expireTimeout) {
    ...
    var now = new Date();
    if(!expireTimeout) expireTimeout = this._opts.expireTimeMiliseconds;
    if(subkey instanceof Array) {
      for(var i = 0; i < subkey.length; ++i)
        this._cache[key][subkey[i]] = {value: value[subkey[i]], expire: now + expireTimeout[subkey[i]]};
    } else {
      this._cache[key][subkey] = {value: value, expire: now + expireTimeout};
    }
}
