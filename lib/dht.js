...
Dht.prototype._expireCache = function (now) {
    ...
    for(var key in this._cache) {
        ...
        for(var subkey in entry) {
            ...
            if(expire < now) {
              console.log(key + " " + subkey + " expired at: " + expire);
              delete entry[subkey];
            }
        }
    }
...
Dht.prototype._replicateCache = function (now) {
    ...
    for(var key in this._cache) {
        var id = Id.fromKey(key);
        for(var subkey in this._cache[key]) {
            var entry = this._cache[key][subkey];
            var replicate = entry.refresh + this._opts.replicateTimeMiliseconds;
            if(replicate < now) {
                this._replicate(key, subkey);
                entry.refresh = now;
...
Dht.prototype.multiset = function (key, subkey, value, cb) {
    ...
    this.iterativeFindNode(Id.fromKey(key), function (err, contacts, id) {
        if (err) return cb(err);
        self._storeToMany(key, subkey, value, contacts, null, cb);
    });
};

// Replicate a key-subkey/value
//
Dht.prototype._replicate = function (key, subkey) {
    var self = this;
    var entry = this._cache[key][subkey];
    this.iterativeFindNode(Id.fromKey(key), function (err, contacts, id) {
        if (err) return;
        var now = new Date();
        self._storeToMany(key, subkey, entry.value, contacts, entry.expire - now);
    });
};

// Store the key/value pair into the specified contacts.
//
Dht.prototype._storeToMany = function (key, subkey, value, contacts, expireTimeout, cb) {
    var self = this;
    asyncMap(contacts, function (contact, cb) {
        self._storeTo(key, subkey, value, contact, expireTimeout, cb);
    }, cb);
};
...
//
Dht.prototype._storeTo = function (key, subkey, value, contact, expireTimeout, cb) {
    if (contact.id.equal(this._routes.id)) {
        this._storeToCache(key, subkey, value, expireTimeout);
...
//
Dht.prototype._storeToCache = function (key, subkey, value, expireTimeout) {
    ...
    if(subkey instanceof Array) {
        for(var i = 0; i < subkey.length; ++i) {
            ...
            this._cache[key][subk] = {value: value[subk], expire: expire, refresh: now};
        }
    } else {
        var expire = expireTimeout ? now + expireTimeout : null;
        this._cache[key][subkey] = {value: value, expire: expire, refresh: now};
    }
}
...
Dht.prototype.peek = function (key, subkey) {
    var val = this._findFromCache(key, subkey || key).value;
...
Dht.prototype.peekall = function (key) {
    var val = this._findFromCache(key, undefined).value;
    if(val === undefined) return null;
    return val;
};
...
Dht.prototype.iterativeFindValue = function (key, subkey, cb) {
    ...
    var seeds = this._routes.find(id, this._opts.concurrency);
    this._routes.markRefreshed(id);
    Lookup.proceed(id, key, subkey, seeds, this._lookupOpts, function(err, contacts, value, expireTimeout, srcContact) {
        ...
        if(storeValue) self._storeTo(key, storeSubKey, storeValue, closestContact, expireTimeout);
        return cb(err, value);
    });
};
...
//
Dht.prototype._findNodeOrValue = function (contact, targetId, key, subkey, cb) {
    if(typeof(cb) != 'function') throw new Error("Invalid callback");
    if(key === undefined) {
      return this._findNode(contact, targetId, cb);
    } else {
      return this._findValue(contact, targetId, key, subkey, cb);
    }
};
...
Dht.prototype._onStore = function (endpoint, payload) {
    ...
    this._storeToCache(payload.key, payload.subkey, payload.value, expire);
};
...
Dht.prototype._findFromCache = function (key, subkey) {
    this._expireCache();
    if (!this._cache.hasOwnProperty(key))          return {value: undefined};
    if (Object.keys(this._cache[key]).length == 0) return {value: undefined};
    if (subkey === undefined || subkey === null)   return cleanupValues(this._cache[key]);
    if (!this._cache[key].hasOwnProperty(subkey))  return {value: undefined};
