...
Dht.prototype.getSeeds = function () {
    var seeds = this._routes.find(this._routes.id, Contact.isAlive);
    if(seeds.length == 0) seeds = this._routes.find(this._routes.id);
    return seeds;
};

Dht.prototype.getRoutes = function () {
    return this._routes;
};
...
Dht.prototype._expireCache = function (now) {
    if(now && typeof now != 'number') throw new Error("Expected now to be a timestamp")
...
Dht.prototype.bootstrap = function (seeds, cb) {
    ...
    function bootstrapSome(endpoint, err, res) {
        --remain;
        console.log("Kad Bootstrap: " + endpoint + " responded, " + remain + " to go");
        if (err) {
            if (remain === 0) return bootstrapLookup();
            return;
        }
        // A node joins the network as follows: (§4.7, Join)
        ...
        if (remain === 0) bootstrapLookup();
    }
    
    function bootstrapLookup(){
        console.log("Kad Bootstrap: all nodes contacted, running iterativeFindNode");
...
Dht.prototype._replicate = function (idkey, subkey) {
    ...
    this.iterativeFindNode(Id.fromHex(idkey), function (err, contacts, id) {
        if (err) return;
        var now = Date.now();
        self._storeToMany(idkey, subkey, entry.value, contacts, entry.expire - now, function(){});
    });
};
...
Dht.prototype._findValue = function (contact, targetId, idkey, subkey, cb) {
    if(typeof(cb) != 'function') throw new Error("Invalid callback");
...
Dht.prototype._onFindValue = function (endpoint, payload) {
    ...
    // FIXME: convert expire from Date to miliseconds
    if(val !== undefined && payload.subkey)
        return {value: val.value, expire: val.timeout};
    var res = this._routes.find(payload.targetId).filter(function(contact){
      // The recipient of a FIND_NODE should never return a triple
      // containing the nodeID of the requestor. (§4.3)
      return !contact.id.equal(payload.id);
    });
    return {value: val.value, expire: val.timeout, contacts: res};
};
...
Dht.prototype._findFromCache = function (idkey, subkey) {
    this._expireCache();
    //console.log("DHT._findFromCache " + idkey + " " + subkey + " " + JSON.stringify(this._cache));
    ...
    return addTimeout(this._cache[idkey][subkey]);
    ...
    function cleanupValues(values) {
        ...
        return addTimeout({value: val, expire: expire});
    }
    
    function addTimeout(res) {
      var now = Date.now();
      if(typeof res.expire == 'number') {
        res.timeout = res.expire - now;
      } else {
        res.timeout = {};
        for(var k in res.expire) {
          res.timeout[k] = res.expire[k] - now;
        }
      }
      return res;
    }
