...
Dht.prototype._expireCache = function (expireTimeMiliseconds, now) {
    expireTimeMiliseconds = expireTimeMiliseconds || this._opts.expireTimeMiliseconds;
    now = now || new Date();
    for(var key in this._cache) {
        var id = Id.fromKey(key);
        var entry = this._cache[key];
        var numNodes = this._routes.countClosestNodes(id);
        var expire = expireTimeMiliseconds;
        if(numNodes > this._opts.bucketSize)
            expire = expire * Math.exp(this._opts.bucketSize / numNodes);
        for(var subkey in entry) {
            if(entry[subkey].time + expire < now) {
              console.log(key + " " + subkey + " expired at: " + (entry[subkey].time + expire));
              delete entry[subkey];
            }
        }
    }
...
Dht.prototype._storeToCache = function (key, subkey, value) {
    ...
    if(subkey instanceof Array) {
      for(var i = 0; i < subkey.length; ++i)
        this._cache[key][subkey[i]] = {value: value[subkey[i]], time: now};
    } else {
      this._cache[key][subkey] = {value: value, time: now};
    }
}
...
Dht.prototype.iterativeFindValue = function (key, subkey, cb) {
    ...
    var seeds = this._routes.find(id, this._opts.concurrency);
    this._routes.markRefreshed(id);
    Lookup.proceed(id, key, subkey, seeds, this._lookupOpts, function(err, contacts, value, srcContact) {
...
Dht.prototype._findFromCache = function (key, subkey) {
    ...
    return this._cache[key][subkey].value;

    function cleanupValues(values) {
        var res = {};
        for(var k in values) {
            res[k] = values[k].value;
        }
        return res;
    }
}
