...
function defaultOptions(opts) {
    ...
    opts.expireTimeMiliseconds = opts.expireTimeMiliseconds || 60 * 60 * 24 + 10;
    opts.refreshTimeMiliseconds = opts.refreshTimeMiliseconds || 60 * 60;
    opts.replicateTimeMiliseconds = opts.replicateTimeMiliseconds || 60 * 60;
    opts.republishTimeMiliseconds = opts.republishTimeMiliseconds || 60 * 60 * 24;
}
...
Dht.prototype.bootstrap = function (seeds, cb) {
    ...
    // A node joins the network as follows: [...] it refreshes all
    // buckets further away than its closest neighbor, which will be in the
    // occupied bucket with the lowest index. (§4.7)
    // FIXME: first bucket is refreshed twice. First time with _bootstrapLookup()
    refreshLoop();
