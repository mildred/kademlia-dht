...
Dht.prototype.bootstrap = function (seeds, cb) {
    // FIXME: check the implementation and the iterativeFindNode algorithm
    // and refresh of buckets
    if (seeds.length === 0)
...
Dht.prototype.multiget = function (key, subkey, cb) {
    ...
    // FIXME: Force getting the key from authoritative source on getall
    if (val && subkey !== undefined)
        return process.nextTick(cb.bind(null, null, val));
    var id = Id.fromKey(key);
    var seeds = this._routes.find(id, this._opts.concurrency);
    Lookup.proceed(id, subkey, seeds, this._lookupOpts, function(err, contacts, value) {
        return cb(err, value);
    });
};
...
/*
    this._lookupKey(key, function (err, id, contacts) {
        ...
        return self._getFrom(id, key, subkey, contacts, cb);
    });
};

Dht.prototype._getFrom = function (id, key, subkey, contacts, cb) {
    var contact = contacts.shift();
    var payload = {id: this._routes.id, targetId: id, key: key, subkey: subkey};
    var self = this;
    this.rpc.findValue(contact.endpoint, payload, function (err, result) {
        if (err || typeof result.value === 'undefined') {
            // FIXME: When an iterativeFindValue succeeds, the initiator must
